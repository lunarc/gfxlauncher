...
import jobs, lrms, remote, settings
...
class SubmitThread(QtCore.QThread):
    def __init__(self, job, cmd="xterm", opengl=False):
...
class SessionWindow(QtGui.QWidget):
    ...
    def update_table(self):
        ...
        if user in self.queue.userJobs:
            self.sessionTable.setRowCount(len(self.queue.userJobs[user].keys()))
        else:
            self.sessionTable.setRowCount(0)
            return

        self.sessionTable.setColumnCount(9)
        self.sessionTable.setHorizontalHeaderLabels(["Id", "Name", "State", "Time", "Requested", "Count", "Nodes", "", ""])

        row = 0 
        for id in self.queue.userJobs[user].keys():
            self.sessionTable.setItem(row, 0, QtGui.QTableWidgetItem(str(id)))
            self.sessionTable.setItem(row, 1, QtGui.QTableWidgetItem(str(self.queue.userJobs[user][id]["name"])))
            self.sessionTable.setItem(row, 2, QtGui.QTableWidgetItem(str(self.queue.userJobs[user][id]["state"])))
            self.sessionTable.setItem(row, 3, QtGui.QTableWidgetItem(str(self.queue.userJobs[user][id]["time"])))
            self.sessionTable.setItem(row, 4, QtGui.QTableWidgetItem(str(self.queue.userJobs[user][id]["timelimit"])))
            self.sessionTable.setItem(row, 5, QtGui.QTableWidgetItem(str(self.queue.userJobs[user][id]["nodes"])))
            self.sessionTable.setItem(row, 6, QtGui.QTableWidgetItem(str(self.queue.userJobs[user][id]["nodelist"])))
    ...
    def info_button_clicked(self):
        ...
        #print(self.queue.userJobs["bmjl"])
...
class GfxLaunchWindow(QtGui.QMainWindow):
    ...
    def on_startButton_clicked(self):
        ...
        self.submitThread = SubmitThread(self.job, self.cmd, self.vgl)
